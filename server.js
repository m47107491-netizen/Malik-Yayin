...
app.post('/api/auth/send-code', otpLimiter, async (req, res) => {
  ...
  const displayName =
    name || db.get('users').find({ email }).value()?.name || email.split('@')[0];
...
app.post('/api/auth/verify-code', (req, res) => {
  ...
  res.json({
    ok: true,
    token,
    user: { email: user.email, name: user.name, license: user.license },
  });
});
...
app.post('/api/payment/paytr/init', auth, async (req, res) => {
  try {
    ...
    const payment_amount = Math.round(Number(process.env.LICENSE_PRICE_TRY || 70) * 100);
    ...
    const user_basket = Buffer.from(
      JSON.stringify([
        [
          `MalikYayın ${process.env.LICENSE_DURATION_DAYS || 30} Günlük Lisans + APK`,
          process.env.LICENSE_PRICE_TRY || '70.00',
          1,
        ],
      ])
    ).toString('base64');
