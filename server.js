...
db.defaults({ users: [], otps: [], orders: [], captchas: [] }).write();
...
// Stateless captcha (Vercel serverless uyumlu — dosyaya yazmaz)
function createCaptcha() {
  const a = crypto.randomInt(2, 12);
  const b = crypto.randomInt(1, 10);
  const op = crypto.randomInt(0, 2) === 0 ? '+' : 'x';
  const answer = op === '+' ? a + b : a * b;
  const expiresAt = Date.now() + 10 * 60 * 1000;
  const payload = `${answer}.${expiresAt}.${crypto.randomBytes(4).toString('hex')}`;
  const sig = crypto.createHmac('sha256', JWT_SECRET).update(payload).digest('hex').slice(0, 24);
  const id = Buffer.from(`${payload}.${sig}`).toString('base64url');
  const question = op === '+' ? `${a} + ${b} = ?` : `${a} x ${b} = ?`;
  return { id, question };
}

function consumeCaptcha(id, answer) {
  if (!id || answer === undefined || answer === null || String(answer).trim() === '') {
    return { ok: false, error: 'Güvenlik sorusunu cevapla.' };
  }
  try {
    const raw = Buffer.from(String(id), 'base64url').toString('utf8');
    const parts = raw.split('.');
    if (parts.length !== 4) return { ok: false, error: 'Güvenlik sorusu geçersiz, yenile.' };
    const [ans, expStr, nonce, sig] = parts;
    const payload = `${ans}.${expStr}.${nonce}`;
    const expect = crypto.createHmac('sha256', JWT_SECRET).update(payload).digest('hex').slice(0, 24);
    if (sig !== expect) return { ok: false, error: 'Güvenlik sorusu geçersiz, yenile.' };
    if (Date.now() > Number(expStr)) return { ok: false, error: 'Güvenlik sorusu süresi doldu, yenile.' };
    const given = String(answer).trim().replace(/\s/g, '');
    if (given !== String(ans)) return { ok: false, error: 'Güvenlik sorusu yanlış.' };
    return { ok: true };
  } catch {
    return { ok: false, error: 'Güvenlik sorusu geçersiz, yenile.' };
  }
}
...
app.get('/api/captcha', (req, res) => {
  ...
  res.json({ ok: true, id: cap.id, question: cap.question });
});
...
app.post('/api/auth/register', otpLimiter, async (req, res) => {
  ...
  const cap = consumeCaptcha(captchaId, captchaAnswer);
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: 'MalikYayın kayıt doğrulama kodun',
      html: otpEmailHtml(name, code),
    });
    res.json({
      ok: true,
      needCode: true,
      message: 'Doğrulama kodu e-postana gönderildi. Kodu girerek kaydı tamamla.',
    });
  } catch (err) {
    console.error('Kayıt mail hatası:', err.message);
    db.get('otps').remove({ email }).write();
