...
function buildTransporter() {
  ...
  // App password'ta bosluk olabilir — kaldir
  const pass = String(process.env.SMTP_PASS || '').replace(/\s+/g, '').trim();
  if (!host || !user || !pass) {
    console.error('SMTP eksik:', { host: !!host, user: !!user, pass: !!pass });
    return null;
  }
...
async function sendOtpMail(to, name, code, purpose) {
  ...
  if (!transporter) {
    const msg = 'SMTP ayarlari eksik (SMTP_HOST / SMTP_USER / SMTP_PASS). Vercel Environment Variables kontrol et.';
    console.error(msg);
  ...
  const subjects = {
    register: 'MalikYayin kayit dogrulama kodun',
    reset: 'MalikYayin sifre sifirlama kodu',
    login: 'MalikYayin giris kodun',
  };
  ...
  try {
    const info = await transporter.sendMail({
      from,
      to,
      subject: subjects[purpose] || 'MalikYayin dogrulama kodu',
      html: otpEmailHtml(name || to.split('@')[0], code),
      text: 'MalikYayin dogrulama kodun: ' + code + ' (10 dakika gecerli)',
    });
    console.log('Mail OK', { to, messageId: info.messageId, response: info.response });
    return { ok: true, messageId: info.messageId };
  } catch (err) {
    console.error('Mail gonderim hatasi:', err && (err.response || err.message), err);
    let detail = err && (err.response || err.message) ? String(err.response || err.message) : 'SMTP hatasi';
    // Sik gorulen Gmail hatalari
    if (/Invalid login|EAUTH|535/i.test(detail)) {
      detail = 'Gmail giris reddetti. SMTP_USER ve uygulama sifresini (SMTP_PASS) kontrol et. Normal sifre degil, App Password olmali.';
    } else if (/ENOTFOUND|ECONNREFUSED|ETIMEDOUT|ESOCKET/i.test(detail)) {
      detail = 'SMTP sunucusuna baglanilamadi. SMTP_HOST=smtp.gmail.com ve SMTP_PORT=465 veya 587 dene.';
    }
...
app.post('/api/auth/register', otpLimiter, async (req, res) => {
  ...
  const payload = {
    ...
    needCode: true,
    otpToken,
    message: 'Dogrulama kodu e-postana gonderildi. Kodu girerek kaydi tamamla.',
  };
...
// ============================================================
app.post('/api/auth/register-verify', otpLimiter, (req, res) => {
  const hp = checkHoneypot(req.body);
  if (!hp.ok) return res.status(400).json({ error: hp.error });
  const email = String(req.body?.email || '').toLowerCase().trim();
...
app.post('/api/auth/send-code', otpLimiter, async (req, res) => {
  ...
  const payload = { ok: true, otpToken, message: 'Sifre sifirlama kodu e-postana gonderildi.' };
  if (mail.debug && process.env.OTP_DEBUG === '1') payload.debugCode = code;
  res.json(payload);
});
...
app.post('/api/admin/app/update', adminAuth, (req, res) => {
  const body = req.body || {};
  const prev = db.get('appConfig').value() || {};
  const next = {
...
app.get('/api/admin/app/update', adminAuth, (req, res) => {
  const cfg = db.get('appConfig').value() || {};
  res.json({ ok: true, appConfig: cfg });
});
